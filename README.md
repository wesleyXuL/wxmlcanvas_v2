...
`new`时增加`destZoom`属性，默认3，(建议取值范围2-5)，取消`destWidth`，`destHeight`属性的作用。destZoom值越大，图片越大，建议不传，使用小程序默认值。
...
注意：目前开发者工具图片裁剪显示可能有bug，以真机效果为准。
...
```javascript
import WxmlCanvas from 'wxmlcanvas_v2'; // 根据具体路径修改，node_modules会被忽略
Page({
    drawImage1 () {
        let self = this;
        this.drawImage1 = new WxmlCanvas({
...
`new WxmlCanvas(options)` 时传入的`options`属性如下：
...
| data-line-clamp | Number | 否 | 文字的最大行数，超出则用 ... 截取 |
| data-maxlength | Number | 否 | 文字的最大个数，超出则用 ... 截取（最好不要用英文单词的，单个字母可以） |
